...
import 'dart:developer'; // Used for logging
...
class _LoginState extends State<Login> {
  ...
  void _handleLogin() async {
    ...
    try {
      ...
      // --- 2. Email Domain Check (As per your requirement) ---
      if (!email.toLowerCase().endsWith(_requiredDomain)) {
        throw AuthException('Login requires a valid student email ending with $_requiredDomain.');
      }
      ...
      // Reloading the user ensures we get the latest email verification status.
      await user.reload();
      if (!user.emailVerified) {
        // Send a new verification link and prompt the user
        await user.sendEmailVerification();
        throw const AuthException('Account not verified. A new link has been sent. Please check your email.');
      }

      // --- 5. Firestore Role Check ---
      final docSnapshot = await _db.collection('users').doc(user.uid).get();

      if (!docSnapshot.exists) {
        throw const AuthException('User data not found in database. Contact support.');
      }

      final savedRole = docSnapshot.data()?['role'] as String? ?? 'unknown';

      if (savedRole != requestedRole) {
        // Format the role for display (e.g., 'driver' -> 'Driver')
        String displayRole = savedRole.substring(0, 1).toUpperCase() + savedRole.substring(1).toLowerCase();
        throw AuthException('You are registered as a $displayRole. Please switch your role selection.');
      }
      ...
      switch (e.code) {
        ...
        case 'wrong-password':
        case 'user-not-found':
        case 'invalid-credential': // Newer Firebase Auth code for invalid email/pass
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          Expanded(
            child: InkWell(
              onTap: _isLoading ? null : () => setState(() => _currentRole = UserRole.passenger),
          ...
          Expanded(
            child: InkWell(
              onTap: _isLoading ? null : () => setState(() => _currentRole = UserRole.driver),
              child: _buildRoleButton(
  ...
  Widget _buildTextField({
      ...
      required TextEditingController controller, // Now uses controller
      bool isPassword = false, // Default is false
    }) {
    return Column(
      ...
      children: [
        ...
        Container(
          ...
          child: TextField(
            ...
            // The onChanged for email field is now implicitly handled by _emailController.text
            // The onChanged for password field is now correctly managed by _passwordController
                    ...
                    children: [
                      ...
                      Align(
                        ...
                        child: Padding(
                          ...
                          child: InkWell(
                            onTap: _isLoading ? null : () => log('Forgot Password clicked'),
                            child: Text(
                              "Forgot Password?",
                              style: TextStyle(
                                color: _isLoading ? const Color(0xFF898686) : const Color(0xFF000000),
                                fontSize: 12,
                                fontWeight: FontWeight.bold,
                              ),
                            ),
                      ...
                      // Login Button (Updated to ElevatedButton)
                      Padding(
                        ...
                        child: ElevatedButton(
                          ...
                          style: ElevatedButton.styleFrom(
                            ...
                            minimumSize: const Size(double.infinity, 48), // Set button size
                            shape: RoundedRectangleBorder(
                              borderRadius: BorderRadius.circular(10),
                            ),
                            disabledBackgroundColor: loginButtonColor.withAlpha(153),
                          ),
                          child: _isLoading
                            ? const SizedBox(
                                width: 20,
